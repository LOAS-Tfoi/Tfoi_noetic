  ...
  #define ROS_DEBUG_TEST_12 0 // local path 성분 체크
  ...
  #define ROS_DEBUG_TEST_11 1 // interval time 체크 
  #define ROS_DEBUG_TEST_12 1 // local path 성분 체크
...
void TebLocalPlannerROS::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros)
{
  ...
  {	
    ...
    simple_goal_sub = nh1.subscribe("/move_base_simple/goal", 1, &TebLocalPlannerROS::simpleGoalCallBack, this);

    multi_goal_sub = nh1.subscribe("/move_base/goal", 1, &TebLocalPlannerROS::multiGoalCallBack, this);
...
void TebLocalPlannerROS::localPlanCallback(const nav_msgs::Path::ConstPtr& msg)
{
  ...
  for(const auto& pose : msg->poses)
  {
    // Add each pose from the received path to the local path
    local_path.poses.push_back(pose);        
    //ROS_INFO("localPlanCallback :  x : %f y : %f z : %f ", x, y, z);
  }

  // Update header information of the local path
  local_path.header.stamp = ros::Time::now();
  local_path.header.frame_id = "map";
}


void TebLocalPlannerROS::simpleGoalCallBack(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
   simple_goal_x = msg->pose.position.x;
   simple_goal_y = msg->pose.position.y;
   simple_goal_yaw = tf2::getYaw(msg->pose.orientation);
}

void TebLocalPlannerROS::multiGoalCallBack(const move_base_msgs::MoveBaseActionGoal::ConstPtr &msg)
{
  multi_goal_x = msg->goal.target_pose.pose.position.x;
...
double TebLocalPlannerROS::goalRadianCaculate(const geometry_msgs::PoseStamped &robot_pose_geo, double & angle_to_heading, double x, double y)
{
    ...
    mx = x;
    my = y;
    Rx = (mx - robot_pose_geo.pose.position.x);
    Ry = (my - robot_pose_geo.pose.position.y);
    //Rx = (simple_goal_x - robot_pose_geo.pose.position.x );
    //Ry = (simple_goal_y - robot_pose_geo.pose.position.y);
    angle_to_heading = std::atan2(Ry, Rx);
    ...
    ROS_WARN("angle Difference : %f", angleDifference);
...
//}
  ...
  // 목적지로 부터 떨어진 로봇의 pose 위치 및 각도 
  geometry_msgs::PoseStamped current_robot_pose;
  bool got_robot_pose = costmap_ros_->getRobotPose(current_robot_pose);

  double angle_to_heading = std::atan2( (multi_goal_x - robot_pose_geo.pose.position.x), (multi_goal_y - robot_pose.pose.position.y ));
  //double angle_to_heading = std::atan2( (simple_goal_x - robot_pose_geo.pose.position.x), (simple_goal_y - robot_pose.pose.position.y ));
...
else
{
    ...
    {      
          ...
          if ( (rotateForDetection( robot_pose_geo , goal_yaw) && frontCheckObs(robot_pose_geo) ) ||     //로봇 현재좌표와 전방시거리점의 tan각도가 설정각도보다 크면 정지하여 회전시켜 각도를 조정한다 로봇 현재위치와 전방시점 협각도가 0.2보다 크고 전방에 장애물 
...
{   
    ...
    {    
      ...
      ROS_WARN("=================");
      ROS_WARN("// goal_Yaw(Rad) : %f", goal_yaw);
      ROS_WARN("=================");
...
//   ROS_INFO("local_path_pose x : %f, y : %f, z : %f", 
