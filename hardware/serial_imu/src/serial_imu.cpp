...
#endif
void publish_imu_data(receive_imusol_packet_t *data, sensor_msgs::Imu *imu_data, std_msgs::Float64MultiArray *RPY_msg);
...
int main(int argc, char** argv)
{
	...
	if(sp.isOpen())
	{
		ROS_INFO_STREAM("/dev/ttyS2 is opened.");
	}
	else
	{
		return -1;
...
//				puts("\033c");
				if(receive_gwsol.tag != KItemGWSOL)
				{
					...
					publish_imu_data(&receive_imusol, &imu_data, &RPY_msg);
