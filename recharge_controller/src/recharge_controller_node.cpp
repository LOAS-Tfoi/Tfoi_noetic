...
namespace recharge_controller_ns
{
      ...
      flag(true)
  {
    ...
    // 创建服务客户端对象
    client_ = nh_.serviceClient<recharge_controller::PathIsVaild>("/move_base/check_path");
...
// 发布倒退回充导航点
  ...
  //  脱离回充
  bool RechargeController::disRechargeCallback(recharge_controller::DisRecharge::Request &req, recharge_controller::DisRecharge::Response &res)
  {
    ...
    recharge_controller::PathIsVaild front_path;
    for(int i = 2; i < 13; i ++) {
      ...
      transformed_point_in.pose.position.x = 0.05 * i;  //55cm
  ...
  //回充
  bool RechargeController::rechargeCallback(recharge_controller::Recharge::Request &req, recharge_controller::Recharge::Response &res)
  {    
    if(bms_charge_done_) {
      res.result = 1;
      return true;
    ...
    // 计算开始回充坐标点的位置，距离充电点正前方1.5m处，方向一致
    temp_point_.pose.position.x = recharge_x_ + std::cos(recharge_yaw_) * recharge_dist_; // recharge_dist_临时坐标点的距离0.8
    temp_point_.pose.position.y = recharge_y_ + std::sin(recharge_yaw_) * recharge_dist_;
    temp_point_.pose.position.z = 0;
    
    bool to_temp_point = goToTempPoint();//发布倒退回充导航电,并导航前往
...
// bms info call back
  ...
  //void RechargeController::chassisInfoCallback(const servcie_call::io_fb::ConstPtr& chassis_info_msg) { //dxs
    //std::lock_guard<std::mutex> lock(mutex_);
...
//执行回充
  void RechargeController::rechargeControl() {
    ...
    double shortest_dist = distanceToLine(A, dir, B); //计算了机器人当前位置与充电桩之间的最短距离。
    
    // 计算机器人需要的线速度和角速度
    double linear_vel = -0.08;//dxs 0.07
    ...
    if (!bms_charge_done_) {
      ...
      // }
      // //往右偏了
      // else if( recharge_point_to_base_link_point.point.y < 0 && shortest_dist > dist_delta_ )
      // {
      //   //ROS_INFO("left moving ! ! !\n");
      //   linear_vel = -0.08;
      //   angular_vel = 0.0;
      //   slipangle = 11;
    ...
    //障碍物检测 检查后退过程在是否有障碍物 dxs
    recharge_controller::PathIsVaild back_path;
    ...
    if (client_.call(back_path)) { 
      if(back_path.response.result == 0 && temp_dist > 0.5)//dxs0.25
      {
    ...
    if(!back_is_safe)
    {
      ...
      //有障碍物，超时
      if( static_cast<double>(time_obs_taltol_ / control_frequency_) > time_out_ ) {
    ...
    {
      charge_time_out_ ++ ;
      //ROS_INFO("time is %f \n", double(charge_time_out_) / double(control_frequency_));
      if( (double(charge_time_out_) / double(control_frequency_)) > 4.5 && !bms_charge_done_)
      {
        ROS_WARN("charging timeout!!! time is %f ", (double(charge_time_out_) / double(control_frequency_)));
        is_charge_failed_ = true;
        charge_time_out_ = 0;
        return;
      }
    ...
    if( shortest_dist > 0.15 || recharge_point_to_base_link_point.point.x > 0.08 || fabs(yaw_diff) > 0.15){
      ...
      ROS_INFO("shortest_dist 、recharge_point_x 、yaw_diff : %f %f %f \n",
      shortest_dist, recharge_point_to_base_link_point.point.x,fabs(yaw_diff) );//dxs
    ...
    if( fabs(angular_fb_) > 15 && ( fabs(linear_vel) > 0.01 || fabs(slipangle) > 3 ) )//dxs angular_fb_ > 10(回充时卡顿，回充有时linear_vel为0了，这里导致) 15
    {
      linear_vel = 0.0;
      angular_vel = 0.0;
    }
...
{
  // 初始化ROS节点
  ros::init(argc, argv, "recharge_controller_node");
